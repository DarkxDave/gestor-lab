...
exports.exportCTCFEForm = async (req, res, next) => {
  try {
    const sample_id = req.query.sample_id;
    if (!sample_id) return res.status(400).send('Parámetro sample_id requerido. Ej: /export/ctcfe-form?sample_id=XYZ');
    const data = await model.getBySampleId(sample_id);
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('CTCFE');
    ws.mergeCells('B1:T1'); ws.getCell('B1').value = 'TRAZABILIDAD Y ANÁLISIS - CT, CF y E.coli'; ws.getCell('B1').alignment = { horizontal:'center' }; ws.getCell('B1').font = { bold:true, size:16 };
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); res.setHeader('Content-Disposition', `attachment; filename="CTCFE_${sample_id}.xlsx"`); await wb.xlsx.write(res); res.end();
  } catch (err) { next(err); }
};
