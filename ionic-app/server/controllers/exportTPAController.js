...
exports.addSheetForSample = async (wb, sample_id, rec) => {
  ...
  // Título principal como en RAM: B1..K1
  ws.mergeCells('B1:K1');
  const mainTitle = ws.getCell('B1');
  mainTitle.value = 'TRAZABILIDAD PESADO Y ANÁLISIS';
  mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
  mainTitle.font = { bold: true, size: 16 };
  mainTitle.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEFEFEF' } };
  ws.getRow(1).height = 22;

  // Subtítulo/código (B2..K2) como RAM
  ws.mergeCells('B2:K2');
  const subTitle = ws.getCell('B2');
  subTitle.value = 'R-INS-MM-M-1-15 /23-08-23';
  subTitle.alignment = { horizontal: 'center', vertical: 'middle' };
  subTitle.font = { bold: true, size: 12 };

  // ID band and ALI label (align like RAM: D3 label, E3..G3 value)
  ws.mergeCells('E3:G3');
  ...
  setBorder(3, 5, 3, 7); // E3..G3
  const aliCell = ws.getCell('D3');
  aliCell.value = 'ALI';
  aliCell.alignment = { horizontal: 'center', vertical: 'middle' };
  aliCell.font = { bold: true };
  setBorder(3, 4, 3, 4);

  // Anchos de columnas (compacto como RAM)
  ws.getColumn(2).width = 28; // B (labels más largos)
  ws.getColumn(3).width = 14; // C
  ws.getColumn(4).width = 14; // D
  ws.getColumn(5).width = 14; // E
  ws.getColumn(6).width = 14; // F
  ws.getColumn(7).width = 18; // G (Responsable legible)
  ws.getColumn(8).width = 22; // H (observaciones)

  // Almacenamiento + Observaciones (ajustado como solicitas)
  // Título Almacenamiento en B5..F5 (más compacto)
  ws.mergeCells('B5:F5');
    ...
    ws.getCell('B5').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
    
    // Lugar de almacenamiento (F18..H) — moved left as requested
    ws.mergeCells('F18:H18');
    ws.getCell('F18').value = 'Lugar de almacenamiento:';
    ws.getCell('F18').alignment = { horizontal: 'left' };
    ws.getCell('F18').font = { bold: true };
    ws.getCell('F18').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };

    const storageRows = [
      ['Freezer 33-M', 'storage_freezer_33m'],
    ...
    storageRows.forEach((row, i) => {
      ...
      ws.getCell(`F${rr}`).value = row[0];
      ws.getCell(`F${rr}`).alignment = { horizontal: 'left' };
      ws.getCell(`H${rr}`).value = check(rec && rec[row[1]]);
      ws.getCell(`H${rr}`).alignment = { horizontal: 'center' };
    });
    setBorder(18, 6, 21, 8); // F18..H21
  ws.getCell('B5').alignment = { horizontal: 'center' };
  ws.getCell('B5').font = { bold: true, size: 12 };

  // Título Observaciones más ancho: H5..K5 y área de texto H6..K9
  ws.mergeCells('H5:K5');
  ws.getCell('H5').value = 'Observaciones:';
  ws.getCell('H5').alignment = { horizontal: 'center', vertical: 'middle' };
  ws.getCell('H5').font = { bold: true };
  ws.getCell('H5').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  ws.mergeCells('H6:K9');

  // Bordes por bloque
  setBorder(5, 2, 9, 7); // B5..G9 (almacenamiento)
  setBorder(5, 8, 9, 11); // H5..K9 (observaciones)
  ...
  ws.getColumn(11).width = 14; // K
  ws.getColumn(12).width = 3;  // L (separador)
  ws.getColumn(13).width = 14; // M
  ws.getColumn(14).width = 14; // N
  ws.getColumn(15).width = 14; // O
  ws.getColumn(16).width = 8;  // P
  ws.getColumn(17).width = 3;  // Q
  ws.getColumn(18).width = 16; // R
  ws.getColumn(19).width = 16; // S
  ...
  ws.getCell('B11').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  ...
  ws.mergeCells('G12:G13'); ws.getCell('G12').value = 'Responsable'; ws.getCell('G12').alignment = { horizontal: 'center', vertical: 'middle', wrapText: true }; ws.getCell('G12').font = { bold: true };
  ...
  ws.mergeCells('B18:E18'); ws.getCell('B18').value = 'Equipos para Pesado:'; ws.getCell('B18').alignment = { horizontal: 'center' }; ws.getCell('B18').font = { bold: true }; ws.getCell('B18').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  const eqRows = [
  ...
  // (El bloque G18..I se retiró; ahora el almacenamiento está en F18..H)

  // Puntas/pipetas desechables para pesado (*) (I18..K)
  const baseP = 18;
  ws.mergeCells(`I${baseP}:K${baseP}`);
  ws.getCell(`I${baseP}`).value = 'Puntas/pipetas desechables para pesado';
  ws.getCell(`I${baseP}`).alignment = { horizontal: 'center', vertical: 'middle' };
  ws.getCell(`I${baseP}`).font = { bold: true };
  ws.getCell(`I${baseP}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };

  ws.mergeCells(`I${baseP + 1}:K${baseP + 1}`);
  ws.getCell(`I${baseP + 1}`).value = '(*)';
  ws.getCell(`I${baseP + 1}`).alignment = { horizontal: 'center', vertical: 'middle' };
  ws.getCell(`I${baseP + 1}`).font = { bold: true };
  ws.getCell(`I${baseP + 1}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };

  ws.getCell(`I${baseP + 2}`).value = '1ml /clave:';
  ws.mergeCells(`J${baseP + 2}:K${baseP + 2}`);
  ws.getCell(`J${baseP + 2}`).value = (rec && rec.clave_1ml) || '';

  ws.getCell(`I${baseP + 3}`).value = '10 ml/clave:';
  ws.mergeCells(`J${baseP + 3}:K${baseP + 3}`);
  ws.getCell(`J${baseP + 3}`).value = (rec && rec.clave_10ml) || '';

  ws.mergeCells(`I${baseP + 4}:I${baseP + 6}`);
  ws.getCell(`I${baseP + 4}`).value = 'Otros:';
  ws.getCell(`I${baseP + 4}`).alignment = { vertical: 'middle' };
  ws.mergeCells(`J${baseP + 4}:K${baseP + 6}`);
  ws.getCell(`J${baseP + 4}`).value = (rec && rec.clave_otros) || '';
  ws.getCell(`J${baseP + 4}`).alignment = { vertical: 'top', wrapText: true };

  setBorder(baseP, 9, baseP + 6, 11);
  ...
  ws.getCell(`B${r0}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  ...
  // Mover el encabezado '10 ml' de E27 a D27 (ajustado a D..E para centrado)
  ws.mergeCells(`D${r0}:E${r0}`); ws.getCell(`D${r0}`).value = '10 ml'; ws.getCell(`D${r0}`).alignment = { horizontal: 'center' }; ws.getCell(`D${r0}`).font = { bold: true };
  // Mover 'Claves Micropipetas' más a la izquierda (de H..K a F..I)
  ws.mergeCells(`F${r0}:I${r0}`); ws.getCell(`F${r0}`).value = 'Claves Micropipetas'; ws.getCell(`F${r0}`).alignment = { horizontal: 'center' }; ws.getCell(`F${r0}`).font = { bold: true };
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    if (mpRight[i]) {
      // Mover 32-M, 75-M, 94-M, 103-M de E28..E31 a D28..D31 y sus checks de F->E
      ws.getCell(`D${rr}`).value = mpRight[i][0];
      ws.getCell(`E${rr}`).value = check(rec && rec[mpRight[i][1]]);
      ws.getCell(`E${rr}`).alignment = { horizontal: 'center' };
    }
  }
  const clavesStart = r0 + 1;
  ws.getCell(`F${clavesStart}`).value = 'Puntas/pipetas 1ml/clave'; ws.mergeCells(`G${clavesStart}:I${clavesStart}`); ws.getCell(`G${clavesStart}`).value = (rec && rec.clave_1ml) || '';
  ws.getCell(`F${clavesStart + 1}`).value = 'Puntas/pipetas 10ml/clave'; ws.mergeCells(`G${clavesStart + 1}:I${clavesStart + 1}`); ws.getCell(`G${clavesStart + 1}`).value = (rec && rec.clave_10ml) || '';
  ws.getCell(`F${clavesStart + 2}`).value = 'Otros'; ws.mergeCells(`G${clavesStart + 2}:I${clavesStart + 2}`); ws.getCell(`G${clavesStart + 2}`).value = (rec && rec.clave_otros) || '';
  setBorder(r0, 2, r0 + maxRows, 6);
  // Alinear borde del bloque de 'Claves Micropipetas' movido a F..I
  setBorder(r0, 6, r0 + 3, 9);

  // Quitar "cuadros" en celdas específicas
  ['D32','E32','F32','D33','E33','F33','D34','E34','F34','F31','K27','K28','K29','K30']
    .forEach(addr => { ws.getCell(addr).border = undefined; });

  // Limpieza
  let rL = r0 + maxRows + 2;
  ws.mergeCells(`B${rL}:K${rL}`); ws.getCell(`B${rL}`).value = 'Limpieza'; ws.getCell(`B${rL}`).alignment = { horizontal: 'center' }; ws.getCell(`B${rL}`).font = { bold: true }; ws.getCell(`B${rL}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  const limpRows = [
  ...
  // Orden requerido: B42 'Desinfectante en aerosol' (F42) y B43 'Otros' (F43:K43)
  ws.mergeCells(`B${rr}:E${rr}`);
  ws.getCell(`B${rr}`).value = 'Desinfectante en aerosol';
  ws.getCell(`F${rr}`).value = check(rec && rec.limpieza_aerosol);
  ws.getCell(`F${rr}`).alignment = { horizontal: 'center' };

  rr++;
  ws.mergeCells(`B${rr}:E${rr}`);
  ws.getCell(`B${rr}`).value = 'Otros';
  ws.mergeCells(`F${rr}:K${rr}`);
  ws.getCell(`F${rr}`).value = (rec && rec.limpieza_otros) || '';
  rr++;
  ...
  let rS = rr + 4;
  ws.mergeCells(`B${rS}:K${rS}`); ws.getCell(`B${rS}`).value = 'Material y equipos de Siembra'; ws.getCell(`B${rS}`).alignment = { horizontal: 'center' }; ws.getCell(`B${rS}`).font = { bold: true }; ws.getCell(`B${rS}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  rS++;
  ...
  // Diluyentes
  let rD = rS + 1;
  ws.mergeCells(`B${rD}:K${rD}`); ws.getCell(`B${rD}`).value = 'Diluyentes'; ws.getCell(`B${rD}`).alignment = { horizontal: 'center' }; ws.getCell(`B${rD}`).font = { bold: true }; ws.getCell(`B${rD}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9D9D9' } };
  rD++;
  const dilRows = [
  ...
  // Borrar cuadros (bordes) adicionales solicitados
  ['B45','C45','D45','E45','B46','C46','D46','E46'].forEach(addr => { ws.getCell(addr).border = undefined; });
  // Quitar cuadros en G37..G41, H37..H41, I37..I41, J37..J41, K37..K41
  for (let col of ['G','H','I','J','K']) {
    for (let row = 37; row <= 41; row++) {
      ws.getCell(`${col}${row}`).border = undefined;
    }
  }
  // Quitar cuadros en G42..K42
  for (let col of ['G','H','I','J','K']) {
    ws.getCell(`${col}42`).border = undefined;
  }
