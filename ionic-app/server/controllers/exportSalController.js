...
exports.exportSalForm = async (req, res, next) => {
  try {
    ...
    if (!sample_id) return res.status(400).send('Parámetro sample_id requerido. Ej: /export/sal-form?sample_id=XYZ');
    const data = await model.getBySampleId(sample_id);
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Sal');
    ws.mergeCells('B1:T1'); ws.getCell('B1').value = 'TRAZABILIDAD Y ANÁLISIS - Sal'; ws.getCell('B1').alignment = { horizontal:'center' }; ws.getCell('B1').font = { bold:true, size:16 };
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); res.setHeader('Content-Disposition', `attachment; filename="Sal_${sample_id}.xlsx"`); await wb.xlsx.write(res); res.end();
  } catch (err) { next(err); }
};
