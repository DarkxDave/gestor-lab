...
exports.exportEnteroForm = async (req, res, next) => {
  try {
    ...
    if (!sample_id) return res.status(400).send('Parámetro sample_id requerido. Ej: /export/entero-form?sample_id=XYZ');
    const data = await model.getBySampleId(sample_id);

    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Entero');

    ws.mergeCells('B1:T1');
    ws.getCell('B1').value = 'TRAZABILIDAD Y ANÁLISIS - ENTERO';
    ...
    ws.mergeCells('C5:T9');
    ws.getCell('C5').value = (data && data.notes) || '';
    ws.getCell('C5').alignment = { vertical: 'top', wrapText: true };

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="Entero_${sample_id}.xlsx"`);
