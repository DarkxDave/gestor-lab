...
exports.save = async (sample_id, { notes } = {}) => {
  ...
  await query(
    `INSERT INTO form_saureus_entries (sample_id, notes, created_at, updated_at)
     VALUES (?, ?, NOW(), NOW())
...
exports.getBySampleId = async (sample_id) => {
  const rows = await query('SELECT * FROM form_saureus_entries WHERE sample_id=?', [sample_id]);
  return rows[0] || null;
};

exports.ensureRow = async (sample_id) => {
  await query(
    `INSERT INTO form_saureus_entries (sample_id, created_at, updated_at)
     VALUES (?, NOW(), NOW())
