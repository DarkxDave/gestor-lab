...
exports.save = async (sample_id, payload) => {
  // Asegurar que el sample existe (1 consulta)
  ...
  // Usar INSERT ... ON DUPLICATE KEY UPDATE en una sola consulta
  const colList = columns.join(', ');
  const placeholders = columns.map(() => '?').join(', ');
  const updateClause = columns.map(c => `${c}=VALUES(${c})`).join(', ');
  
  await query(
    `INSERT INTO form_tpa_entries (sample_id, ${colList}, created_at, updated_at) 
...
exports.ensureRow = async (sample_id) => {
  await query(
    `INSERT INTO form_tpa_entries (sample_id, created_at, updated_at)
     VALUES (?, NOW(), NOW())
